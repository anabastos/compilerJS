...
const hashTable = (debug = 'N') => {
  ...
  const checkDebug = (key, table, hash, action) =>
    debug === 'N' ||
    console.log(
      `Simbolo:${key} Hashing:${hash} Posição:${hash} Ação:${action}`
    )
...
// le_tst_binario
const readFromBinary = async (path) => await readFilePromisified(path)
...
export default { readFromBinary, hashTable, test, createPopulatedTable }
